...
import { CalendarDays, LayoutGrid, Leaf, Moon, RotateCcw, Tag, Trash2 } from 'lucide-react-native';
...
export default function ProfileScreen() {
  ...
  return (
    <LargeTitleScreen title="我的">
      <GroupedSection><View style={styles.hero}><View style={[styles.avatar, { backgroundColor: LIME }]}><Text style={styles.avatarGlyph}>衡</Text></View><View style={styles.heroText}><Text style={[styles.name, { color: c.text }]}>衡物</Text><View style={[styles.localPill, { backgroundColor: c.chip }]}><View style={[styles.localDot, { backgroundColor: isSignedIn ? c.success : c.textTertiary }]} /><Text style={[styles.localText, { color: c.textSecondary }]}>{isSignedIn ? '已登录 · Clerk' : '本地账本'}</Text></View></View></View></GroupedSection>
      ...
      {isSignedIn ? <GroupedSection header="云端" inset={GROUPED_INSETS.plain} footer="一键迁移会用本机 hengwu-db 全量替换云端。"><View style={{ paddingHorizontal: 16, paddingVertical: 12 }}><MigratePanel /></View><GroupedRow plain label="退出登录" destructive onPress={() => { void signOut(); }} /></GroupedSection> : <GroupedSection header="云端" inset={GROUPED_INSETS.plain}><GroupedRow plain label="登录以同步云端" onPress={() => router.push('/login')} /></GroupedSection>}
